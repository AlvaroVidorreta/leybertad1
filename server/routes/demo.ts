...
export const handleDemo: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (!apiKey)
      return res
        .status(500)
        .json({ ok: false, error: "Missing Firebase API key" });
    ...
    if (!idToken)
      return res
        .status(500)
        .json({ ok: false, error: "No idToken in response", details: body });
    ...
    if (action === "auth-check" && run !== "protected") {
      return res.json({
        ok: true,
        uid: decoded.uid,
        token_issued_at: decoded.iat || null,
      });
    }
    ...
    try {
      ...
      const createdLaw = await db.createLaw(
        {
          titulo: "Prueba integracion",
          objetivo: "Test automático",
          apodo: "auto",
        },
        visitorKey,
      );
      ...
      try {
        const commented = await db.commentLaw(
          lawId,
          "Comentario de prueba automatizado",
          decoded.uid,
        );
