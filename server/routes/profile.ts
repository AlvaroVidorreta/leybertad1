...
export const profileHandler: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const displayName = (persisted && persisted.displayName) || defaultDisplayName;
...
export const profileUpdateHandler: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (typeof body.displayName === 'string') patch.displayName = String(body.displayName).slice(0, 100);
    if (typeof body.username === 'string') patch.username = String(body.username).slice(0, 60);
