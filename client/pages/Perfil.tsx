...
export default function Perfil() {
  ...
  const [tab, setTab] = useState<"creaciones" | "actividad" | "guardados">(
    "creaciones",
  );
      ...
      try {
        ...
        try {
          ...
          if (raw) {
            ...
            const merged = {
              ...(pRes as ProfileResponse),
              displayName:
                parsed.displayName || (pRes as ProfileResponse).displayName,
              username: parsed.username || (pRes as ProfileResponse).username,
            } as ProfileResponse;
            setProfile(merged);
          } else {
  ...
  const savedLaws = useMemo(
    () => allLaws.filter((l) => savedIds.includes(l.id)),
    [allLaws, savedIds],
  );

  const registered = useMemo(
    () => !!profile && !profile.displayName?.startsWith("Usuario "),
    [profile],
  );
      ...
      <section className="flex items-center gap-6 py-6 border-b mb-6">
        ...
        <div className="flex-1">
          <h2 className="font-brand text-2xl text-primary">
            {registered ? profile!.displayName : "Usuario no registrado"}
          </h2>
          <div className="text-sm text-muted-foreground">
            {registered ? profile!.username : "Usuario no registrado"}
          </div>
          ...
          <div className="mt-3 flex gap-4 text-sm text-muted-foreground">
            <div>
              ...
              <span className="text-foreground font-semibold">
                {profile ? profile.created.length : 0}
              </span>
            </div>
            <div>
              Votos emitidos:{" "}
              <span className="text-foreground font-semibold">
                {profile ? profile.voted.length : 0}
              </span>
            </div>
          </div>
        </div>
      ...
      <section className="py-6 border-b mb-6 text-center">
        ...
        <div className="mt-4">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Calculado en base a las propuestas de ley que has apoyado.
          </p>
        </div>
      </section>
            ...
            {!loading &&
              ...
              profile.created.slice(0, 5).map((law) => (
                <a
                  key={law.id}
                  href={`/laws/${law.id}`}
                  className="block border rounded-md p-3 hover:shadow-sm flex justify-between items-center"
                >
                  <LawSummary
                    title={law.titulo}
                    objetivo={law.objetivo}
