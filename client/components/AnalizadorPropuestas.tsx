...
export default function AnalizadorPropuestas({
  ...
  return (
    <div
      className={`rounded-2xl border border-stone-700 bg-stone-900/60 backdrop-blur p-4 md:p-5 text-white flex flex-col min-h-0 overflow-hidden ${results === null ? "max-h-[24vh]" : "max-h-[36vh]"}`}
    >
      <div
        ...
        {results !== null && (
          <div className="flex-1 min-h-0 flex flex-col">
            <div className="flex-1 min-h-0 overflow-auto">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-sm px-3 py-1 rounded-md border border-stone-700 bg-stone-800/50 hover:bg-stone-800 flex items-center gap-2 text-stone-200"
                  ...
                  {showFilter &&
                    ...
                    ReactDOM.createPortal(
                      <div
                        ...
                        className="rounded-md shadow-lg bg-stone-800 border border-stone-700 text-white z-50"
                      >
                        <button
                          className={`w-full text-left px-3 py-2 ${timeframe === "any" ? "bg-stone-700" : ""}`}
                          onClick={() => {
                            setTimeframe("any");
                        ...
                        <button
                          className={`w-full text-left px-3 py-2 ${timeframe === "week" ? "bg-stone-700" : ""}`}
                          onClick={() => {
                            setTimeframe("week");
                        ...
                        <button
                          className={`w-full text-left px-3 py-2 ${timeframe === "month" ? "bg-stone-700" : ""}`}
                          onClick={() => {
                            setTimeframe("month");
                        ...
                        <button
                          className={`w-full text-left px-3 py-2 ${timeframe === "year" ? "bg-stone-700" : ""}`}
                          onClick={() => {
                            setTimeframe("year");
              ...
              {!hasResults && (
                <div className="text-center text-sm text-stone-300 py-6">
              ...
              {hasResults && (
                <ul className="space-y-2 p-1">
                  {results!.map((r) => (
                    <li
                      key={r.law.id}
                      className="rounded-lg border border-stone-700 bg-stone-900/30 p-2"
                    >
                      <div className="flex items-center">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0 flex flex-col justify-center space-y-1 translate-y-px">
                              <h6 className="font-medium text-sm text-stone-50 truncate">
                                {r.law.title}
                              </h6>
                              <p className="italic text-xs text-stone-300 line-clamp-2">
                                {r.law.summary}
                              </p>
                            </div>
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="ml-2 whitespace-nowrap text-sm text-stone-400 hover:text-stone-300 underline-offset-4 hover:underline flex items-center py-2 self-center"
                            >
                              Consultar →
                            </a>
                          </div>
