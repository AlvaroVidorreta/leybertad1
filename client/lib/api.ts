...
async function safeFetch(path: string, opts?: RequestInit) {
  ...
  const err = new Error(
    `Network fetch failed for ${path}. Tried ${candidates.join(", ")}`,
  );
...
export async function votarLey(id: string): Promise<Law> {
  const res = await safeFetch(`/api/laws/${id}/upvote`, {
    method: "POST",
...
export async function comentarLey(
  id: string,
  input: CommentInput,
): Promise<Law> {
